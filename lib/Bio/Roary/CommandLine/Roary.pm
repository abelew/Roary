...
use Bio::Roary::ReformatInputGBs;
...
has 'check_dependencies'      => ( is => 'rw', isa => 'Bool', default => 0 );
...
sub BUILD {
    ...
    my (
        ...
        $mafft,                 $output_directory,  $check_dependencies, $inflation_value, $allow_paralogs,
        );
    ...
    GetOptionsFromArray(
        ...
        'a|check_dependencies'      => \$check_dependencies,
    ...
    $self->check_dependencies($check_dependencies) if (defined($check_dependencies));
...
sub run {
    ...
    if ($self->version || $self->help ||
        ($self->check_dependencies && @{$self->fasta_files} < 1)) {
        return;
    }
    $self->_setup_output_directory;

    my @test_files = @{$self->fasta_files};
    my $first_file = $test_files[0];
    my ($first_name, $first_path, $first_suffix) = fileparse($first_file, qr/\.[^.]*/);
    my $reformat_input_files;
    print "TESTME: FIRST SUFFIX: $first_file , $first_name ,  $first_path ,  $first_suffix\n";
    if ($first_suffix eq '.gb') {
        $self->logger->info("Assuming the inputs are Genbank files.");
        $reformat_input_files = Bio::Roary::ReformatInputGBs->new(gb_files => $self->fasta_files,
                                                                  logger => $self->logger);
    } else {
        $self->logger->info("Fixing input GFF files");
        $reformat_input_files = Bio::Roary::ReformatInputGFFs->new(gff_files => $self->fasta_files,
                                                                   logger => $self->logger);
    }
    $reformat_input_files->fix_duplicate_gene_ids();
    if (@{$reformat_input_files->fixed_input_files} == 0) {
        die(
    ...
    $self->fasta_files($reformat_input_files->fixed_input_files);
...
sub usage_text {
  ...
  Usage:   roary [options] *.gff
    ...
    Options: -p INT    number of threads [1]
      ...
      -a        check dependencies and print versions
